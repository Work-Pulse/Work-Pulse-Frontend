...
const TaskManager = () => {
  ...
  const addUser = () => {
    ...
    const newUser = { id: userId.trim(), name: userName.trim() };
  ...
  const addTask = () => {
    ...
    if (!taskName.trim() || (durationHours === "" && durationMinutes === "") || !deadline) return;
  ...
  return (
    ...
    >
      ...
      <div className="bg-white p-6 rounded-lg shadow-lg border-2 border-gray-300">
          ...
          {users.length > 0 && (
            <select className="w-full mt-4 p-2 border rounded shadow-sm max-h-40 overflow-auto" onChange={(e) => selectUser(e.target.value, users.find(user => user.id === e.target.value)?.name || "") }>
        ...
        <div className="bg-white p-6 rounded-lg shadow-lg border-2 border-gray-300">
          ...
          <div className="flex gap-3 mb-3">
            <select value={priority} onChange={(e) => setPriority(e.target.value as "High" | "Medium" | "Low")} className="w-1/2 p-2 border rounded">
              <option value="High">High ★★★</option>
              <option value="Medium">Medium ★★</option>
              <option value="Low">Low ★</option>
            </select>
            <input type="number" placeholder="Hours" value={durationHours} onChange={(e) => setDurationHours(e.target.value)} className="w-1/4 p-2 border rounded" />
            <input type="number" placeholder="Mins" value={durationMinutes} onChange={(e) => setDurationMinutes(e.target.value)} className="w-1/4 p-2 border rounded" />
          </div>
          <label className="text-text font-bold">Deadline</label>
          <input type="date"  value={deadline} onChange={(e) => setDeadline(e.target.value)} className="w-full p-2 border rounded mb-3 shadow-sm" />
          <button onClick={addTask} className="w-full p-3 bg-[#122D3B] text-white rounded-lg hover:bg-blue-700 transition duration-300 shadow-md">Add Task</button>
        </div>
      </div>
        ...
        {selectedUser && userTasks[selectedUser.id]?.length > 0 ? (
          <ul className="space-y-3">
            {userTasks[selectedUser.id].map((task) => (
              <li key={task.id} className="p-3 bg-gray-100 border rounded-lg flex justify-between">
                <div>
                  <p className="font-bold">{task.name} {getPriorityStars(task.priority)}</p>
