...
const TaskManager = () => {
  ...
  const addTask = () => {
    ...
    if (!taskName.trim() || (durationHours === "" && durationMinutes === "")) return;
    ...
    >
      ...
      <div className="grid grid-cols-2 gap-6 w-full max-w-5xl p-6 bg-gray-100 rounded-lg shadow-xl">
        ...
        <div className="bg-white p-6 rounded-lg shadow-lg border-2 border-gray-300">
          ...
          <textarea placeholder="Task Description (optional)" value={description} onChange={(e) => setDescription(e.target.value)} className="w-full p-2 border rounded mb-3 h-20 resize-none shadow-sm" /><div className="flex gap-3 mb-3">
          <select 
            value={priority} 
      ...
      <div className="w-full max-w-5xl p-6 bg-white rounded-lg shadow-lg border-2 border-gray-300 mt-6 overflow-auto">
        <h3 className="text-xl font-bold text-[#122D3B] mb-3">Task List</h3>
            ...
            {userTasks[selectedUser.id].map((task) => (
              <li key={task.id} className="p-3 bg-gray-100 border rounded-lg flex justify-between">
                <div>
                  ...
                  <p className="text-base font-semibold text-gray-600">Priority: {task.priority}</p>
                  <p className="text-base font-semibold text-gray-600">Duration: {task.duration}</p>
                </div>
